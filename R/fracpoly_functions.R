...
#'
#' @export
plot.hazard.ratios <- function(hr, reftrt, treats=attr(hr, "trtnames")) {
  ...
  # Subset treats
  out.df <- subset(out.df, trt1 %in% treats)
...
#' @param integrate Method for integrating over cumulative hazard function. Can take either
...
survcalc <- function(jagsmod, refstudy, refmod=jagsmod,
                     times=seq(1,60, length.out=100),
                     n.mcmc=NULL, d.weight=NULL, integrate="trapezoid") {
  ...
  for (k in 1:jagsdat$nt) {
    ...
    } else if (integrate=="quadrature") {
      #H <- matrix(ncol=length(times), nrow=nrow(beta))
      H <- matrix(nrow=length(times), ncol=nrow(beta))
      pb <- txtProgressBar(min = 0, max = length(times), initial = 0)
      for (m in 1:nrow(H)) {

        H[m,]  <- apply(beta, MAR = 1, function(y) {cumulative_hazard_function(t=times[m],
                                                                               params=y,
                                                                               lower=0.001,
                                                                               exponents=exponents,
                                                                               method="pracma")})
    ...
    # dH_sum <- t(apply(dH, MAR = 1, FUN = mcmc_sum))
    # temp <- data.frame(time = times,
    #                    treatment = k,
    #                    dH_sum)
    # dH.df <- rbind(dH.df, temp)
  ...
  out <- list(haz=haz.df, cumhaz=cumhaz.df, #dH=dH.df,
              mort=mort.df, S=S.df,
...
#' @param treats treatments to plot. Default plots them all
...
plot.surv.predicts <- function(surv, quantity="S",
  ...
  # Generate linetypes
  all.linet <- c("solid", "dashed", "dotted", "dotdash",
                 "longdash", "twodash",
                 "44", "13", "5212", "3313", "11", "2226")
  linet <- all.linet[unique(as.numeric(out.df$treatment))]
  ...
  g <- ggplot2::ggplot(out.df, ggplot2::aes(x=time, ymin=`2.5%`, ymax=`97.5%`, y=`50%`,
    ...
    ggplot2::scale_fill_manual(name = "Treatment", values=cols) +
    ggplot2::scale_color_manual(name = "Treatment", values=cols) +
    ggplot2::scale_linetype_manual(name = "Treatment", values=linet) +
